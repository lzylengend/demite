...
func (this *_RemoteScheduleDao) ListByRemoteId(remoteId int64) ([]*RemoteSchedule, error) {
	objList := make([]*RemoteSchedule, 0)

	err := this.Db.Where("repairid = ? and datastatus = ? ", remoteId, 0).Order("createtime").Find(&objList).Error
